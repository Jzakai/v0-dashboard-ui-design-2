...
export function Sidebar({ userRole, onLogout, onNavigate }: SidebarProps) {
  ...
  const adminItems = [
    ...
    { id: 'courses', label: 'Courses', icon: '📚', description: 'Manage all' },
    { id: 'create-course', label: 'Create Course', icon: '✏️', description: 'New scenario' },
    { id: 'assign-courses', label: 'Assignments', icon: '👥', description: 'Assign to trainees' },
    { id: 'analytics', label: 'Analytics', icon: '📈', description: 'AAR reports' },
  ];

  const traineeItems = [
    { id: 'dashboard', label: 'Dashboard', icon: '📊', description: 'My overview' },
    { id: 'my-trainings', label: 'My Trainings', icon: '🎯', description: 'View courses' },
    { id: 'my-results', label: 'My Results', icon: '📈', description: 'Track progress' },
  ];
    ...
    <aside className="w-72 bg-sidebar border-r border-sidebar-border h-screen flex flex-col fixed left-0 top-0 shadow-xl">
      ...
      <div className="p-6 border-b border-sidebar-border bg-gradient-to-r from-primary/10 to-accent/10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-lg font-bold text-sidebar-foreground">VR Paramedics</h1>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="flex-1 text-left">
              ...
              <div className={cn(
                "text-xs mt-0.5",
                activeItem === item.id ? 'text-white/80' : 'text-muted-foreground'
